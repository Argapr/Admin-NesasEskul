...
async function addDataToFirestore(name, image, pengumuman) {
  try {
...
const Pengumuman = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const added = await addDataToFirestore(name, image, pengumuman);
    if (added) {
      setName("");
      setImage("");
      setPengumuman("");
                ...
                <tbody>
                  {pengumumanData.map((pengumuman, index) => (
                    <tr key={pengumuman.id} className={index % 2 === 0 ? "bg-transparent" : "bg-gray-200"}>
                      ...
                      <td className="border px-4 py-2">{pengumuman.image}</td>
